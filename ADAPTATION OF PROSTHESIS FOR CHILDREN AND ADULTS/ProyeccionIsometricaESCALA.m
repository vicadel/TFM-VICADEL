...
figure;
hold on;
title('Modelos de Prótesis Escaladas (Isométrica)', 'Color', 'k');
xlabel('Longitud (mm)', 'Color', 'k');
ylabel('Anchura (mm)', 'Color', 'k');
zlabel('Altura (mm)', 'Color', 'k');
grid on;

% Añadir modelos al gráfico isométrico con escalas
plot3([0, baby_length], [0, baby_width], [0, baby_height], 'c', 'LineWidth', 2);
text(baby_length, baby_width, baby_height, ...
    sprintf('Bebé\nL: %d\nA: %d\nH: %d', baby_length, baby_width, baby_height), 'Color', 'k');

plot3([0, child_length], [0, child_width], [0, child_height], 'b', 'LineWidth', 2);
text(child_length, child_width, child_height, ...
    sprintf('Niño/Niña\nL: %d\nA: %d\nH: %d', child_length, child_width, child_height), 'Color', 'k');

plot3([0, teen_male_length], [0, teen_male_width], [0, teen_male_height], 'g', 'LineWidth', 2);
text(teen_male_length, teen_male_width, teen_male_height, ...
    sprintf('Adolescente Hombre\nL: %d\nA: %d\nH: %d', teen_male_length, teen_male_width, teen_male_height), 'Color', 'k');

plot3([0, teen_female_length], [0, teen_female_width], [0, teen_female_height], 'm', 'LineWidth', 2);
text(teen_female_length, teen_female_width, teen_female_height, ...
    sprintf('Adolescente Mujer\nL: %d\nA: %d\nH: %d', teen_female_length, teen_female_width, teen_female_height), 'Color', 'k');

plot3([0, adult_male_length], [0, adult_male_width], [0, adult_male_height], 'r', 'LineWidth', 2);
text(adult_male_length, adult_male_width, adult_male_height, ...
    sprintf('Adulto Hombre\nL: %d\nA: %d\nH: %d', adult_male_length, adult_male_width, adult_male_height), 'Color', 'k');

legend('Bebé', 'Niño/Niña', 'Adolescente Hombre', 'Adolescente Mujer', 'Adulto Hombre');
view(45, 45); % Vista isométrica
hold off;
